...
public class MarqueeLabel: UILabel {
    ...
    @IBInspectable public var labelize: Bool = false {
        didSet {
            guard labelize != oldValue else { return }
            updateAndScroll()
        }
    }
    ...
    @IBInspectable public var holdScrolling: Bool = false {
        didSet {
            guard holdScrolling != oldValue else { return }
    ...
    @IBInspectable public var tapToScroll: Bool = false {
        didSet {
            guard tapToScroll != oldValue else { return }
            if tapToScroll {
    ...
    @IBInspectable public var leadingBuffer: CGFloat = 0.0 {
        didSet {
            guard leadingBuffer != oldValue else { return }
            updateAndScroll()
        }
    }
    ...
    @IBInspectable public var trailingBuffer: CGFloat = 0.0 {
        didSet {
            guard trailingBuffer != oldValue else { return }
            updateAndScroll()
        }
    }
    ...
    private func setup() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(MarqueeLabel.restartForViewController(_:)), name: MarqueeKeys.restart.notification, object: nil)
    ...
    override public func layoutSubviews() {
        ...
        updateAndScroll(shouldBeginScroll: true)
    }
    ...
    private func beginScroll() {
        beginScroll(afterDelay: true)
    }
    
    private func beginScroll(afterDelay: Bool) {
        switch self.type {
    ...
    private func scroll(_ interval: CGFloat, delay: CGFloat = 0.0, scroller: Scroller, fader: CAKeyframeAnimation?) {
        ...
        // Check for IBDesignable
            ...
            if fadeLength > 0.0 {
                ...
                if let previousAnimation = fader {
                    ...
                    gradientAnimation = self.gradientKeyFrameAnimation(for: fadeLength, interval: interval, delay: delay)
                }
        ...
        scrollCompletionBlock = { [weak self] (finished: Bool) -> () in
            ...
            self!.labelReturnedToHome(finished: true)
    ...
    private func scrollAway(_ interval: CGFloat, delay: CGFloat = 0.0) {
        ...
        let awayOrigin = offset(homeLabelFrame.origin, dx: awayOffset)
    ...
    private func gradientKeyFrameAnimation(for fadeLength: CGFloat, interval: CGFloat, delay: CGFloat) -> CAKeyframeAnimation {
        // Setup
        let values: [[CGColor]]
        ...
        default:
            ...
            let startFadeTimeFraction = timingFunction.durationPercentage(for: startFadeFraction, duration: totalDuration)
    ...
    class private func notifyController(_ controller: UIViewController, message: MarqueeKeys) {
        NotificationCenter.default.post(name: message.notification, object: nil, userInfo: ["controller": controller])
    }
    ...
    public override var text: String? {
        get {
            ...
            guard sublabel.text != newValue else { return }
    ...
    public override var attributedText: NSAttributedString? {
        get {
            ...
            guard sublabel.attributedText != newValue else { return }
    ...
    public override var font: UIFont! {
        get {
            ...
            guard sublabel.font != newValue else { return }
    ...
    private func offset(_ point: CGPoint, dx xOffset: CGFloat = 0.0, dy yOffset: CGFloat = 0.0) -> CGPoint {
...
extension CAMediaTimingFunction {
    ...
    func durationPercentage(for positionPercentage: CGFloat, duration: CGFloat) -> CGFloat {
        ...
        let t_found = solveT(for: positionPercentage, epsilon: epsilon, in: controlPoints)
        
        // With that t, find the corresponding animation percentage
        let durationPercentage = findX(forCurveAt: t_found, in: controlPoints)
        
        return durationPercentage
    }
    
    func solveT(for y: CGFloat, epsilon: CGFloat, in controlPoints: [CGPoint]) -> CGFloat {
        ...
        for _ in 0..<15 {
            ...
            t0 = t1
            // Calculate f(t0)
            f0 = findY(forCurveAt: t0, in: controlPoints) - y
            ...
            df0 = derivativeCurve(yValueAt: t0, in: controlPoints)
    ...
    func findY(forCurveAt t: CGFloat, in controlPoints:[CGPoint]) -> CGFloat {
    ...
    func findX(forCurveAt t: CGFloat, in controlPoints: [CGPoint]) -> CGFloat {
        let P0 = controlPoints[0]
        let P1 = controlPoints[1]
        let P2 = controlPoints[2]
        let P3 = controlPoints[3]
    ...
    func derivativeCurve(yValueAt t: CGFloat, in controlPoints: [CGPoint]) -> CGFloat {
        let P0 = controlPoints[0]
        let P1 = controlPoints[1]
        let P2 = controlPoints[2]
        let P3 = controlPoints[3]
